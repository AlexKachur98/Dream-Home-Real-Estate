...
## Project Overview
Dream Home Real Estate is our capstone-style build that pairs a React/Vite front-end with an Express API wired for Oracle stored procedures. The current milestone delivers a fully documented UI skeleton covering the instructor’s Staff, Branch, Client, and Authentication menus, while the backend team prepares the database integrations.
...
├─ docs/            # Instructor brief and supporting assets
└─ package.json     # npm workspace entry point + shared scripts
```

### Front-End Highlights (client/)
- React Router 7 navigation with documented routes for staff, branch, client, login, and registration flows.
...
- Front-end components call `resolvePlaceholder(...)` so they render meaningful layouts before APIs are live.
